...
TELEMETRY_DATA_FILE = ".revelio/telemetry.txt"
...
def telemetry_opt_out():
    return os.getenv("REVELIO_TELEMETRY_OPT_OUT") == "YES"
...
def get_status() -> str:
    ...
    return data.get("REVELIO_STATUS", "new")


def get_unique_id() -> str:
    """Gets or generates a unique ID and updates the telemetry file."""
    data = read_telemetry_file()
    unique_id = data.get("REVELIO_ID")
    if not unique_id:
        unique_id = str(uuid.uuid4())
        data["REVELIO_ID"] = unique_id
        data["REVELIO_STATUS"] = "new"
    else:
        data["REVELIO_STATUS"] = "old"
...
def get_last_feature() -> Feature:
    ...
    data = read_telemetry_file()
    last_feature = data.get("REVELIO_LAST_FEATURE")
...
def set_last_feature(feature: Feature):
    ...
    data = read_telemetry_file()
    data["REVELIO_LAST_FEATURE"] = feature.value
    feature_status_key = f"REVELIO_{feature.value.upper()}_STATUS"
